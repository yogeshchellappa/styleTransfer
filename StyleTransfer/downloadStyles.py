...
def downloadImages(theme, extension, numberOfImages = 5, height = 300, width = 400):
	...
	while j<len(keywords):
	    ...
	    time.sleep(0.1)
	    items = items + (getAllImageItems(raw_html,extension,numberOfImages))
	    j = j + 1

	for i in range(numberOfImages):
		http = urllib3.PoolManager()
		req = http.request('GET', items[i], headers={"User-Agent": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"})
		response = req.data
		imageName = imagePath + "style" + str(i+1) + "." + extension
